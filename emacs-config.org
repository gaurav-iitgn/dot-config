...
#+BEGIN_SRC elisp
(defun gs/debug-print (MSG)
"Prints a message with timestamp for easy debugging."
  (message "%s: %s\n" (format-time-string "%H:%M:%S" (current-time)) MSG)
)
#+END_SRC
...
"Gets the name of the document resource directory of the current headline within the org file"
  ...
  (setq mybasedir *gs-google-d*)
  ;;set full base directory depending on the file location identifier
  (cond 
    ((equal mydocdirstr "google")(setq mybasedir *gs-google-d*))
...
"Gets the name of the document resource directory of the current headline within the org file"
  ...
  (let ((mydocdirstr (car (org-property-values "DOCDIR")))
    ...
    ; corresponding directory name will be 405.2.1_name
	  ...
	  (gs/debug-print (concat ".mydocdirstr: " mydocdirstr))
  ...
  (setq mydir (concat mybasedir "/" mytag "_*"))
	...
	(gs/debug-print (concat ".mydir (after building it): " mydir))
	
  ;; expand the wildcard to get full name
	;; file-expand-wildcards returns a list; pick the first element with car
  (setq mydir (car (file-expand-wildcards mydir)))
	
	(gs/debug-print (concat ".mydir (after wildcard expansion): " mydir))

  ; this will return the final system-dependent path
  (setq mydir (concat "'" (expand-file-name mydir) "'"))
  ...
  (if *sys-win32*
	...
	;(message "?mydir after system dependent path: %s." mydir)
...
"Opens document resource directory of the current headline within the org file"
  (interactive)
  (setq ord-cmd (concat *gs-file-manager* (gs/get-resource-dir-try)))
	(gs/debug-print (concat ".ord-cmd: " ord-cmd))
  (shell-command ord-cmd)
  ;(shell-command (concat *gs-file-manager* (gs/get-resource-dir)))
